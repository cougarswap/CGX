...
contract CGXAuto is Ownable, Pausable {
    ...
    function harvest() external notContract whenNotPaused {
        IPancakeswapFarm(masterchef).withdraw(CGXPID, 0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        IPancakeswapFarm(masterchef).emergencyWithdraw(CGXPID);
    }
     ...
     */
    function calculateHarvestCGXRewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingCGX(CGXPID, address(this));
     ...
     */
    function calculateTotalPendingCGXRewards() external view returns (uint256) {
        uint256 amount = IPancakeswapFarm(masterchef).pendingCGX(CGXPID, address(this));
        amount = amount.add(available());
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            IPancakeswapFarm(masterchef).withdraw(CGXPID, balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = IPancakeswapFarm(masterchef).userInfo(CGXPID, address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IPancakeswapFarm(masterchef).deposit(CGXPID, bal, treasury);
        }
    }
